...
server.get('/vendedores/:id_user', async (req, res) =>{
 ...
 server.post('/vendedores', async (req, res) => {
   ...
   if (hasEmail.length === 0){
    const result = await db.pool.query("insert into vendedor (nome,cnpj, nome_fantasia, contato, numero, rua, cep, bairro, cpf, email, cidade, uf, senha)values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [vendedor.nome, vendedor.cnpj, vendedor.nome_fantasia, vendedor.contato, vendedor.numero, vendedor.rua, vendedor.cep, vendedor.bairro, vendedor.cpf, vendedor.email, vendedor.cidade, vendedor.uf, vendedor.senha]);
    res.send(JSONBig.parse(JSONBig.stringify(result)));
   } else{
 ...
 server.put('/vendedores/:id_vendedor', async (req, res) => {
   ...
   const result = await db.pool.query("update vendedor set nome=?, cnpj=?, nome_fantasia=?, email=?, contato=?, numero=?, rua=?, cep=?, bairro=?, cpf=?, cidade=?, uf=?, where id_vendedor = ?", [vendedor.nome, vendedor.cnpj, vendedor.nome_fantasia, vendedor.email, vendedor.contato, vendedor.numero, vendedor.rua, vendedor.cep, vendedor.bairro, vendedor.tipo, vendedor.cpf, vendedor.cidade, vendedor.uf, headerProduto]);
   res.send(JSONBig.parse(JSONBig.stringify(result)));
 });
